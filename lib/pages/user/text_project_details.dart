...
class TextProjectShowCase extends StatefulWidget {
  final String projectId;

  const TextProjectShowCase({super.key, required this.projectId});

  @override
  State<TextProjectShowCase> createState() => _TextProjectShowCaseState();
}

class _TextProjectShowCaseState extends State<TextProjectShowCase> {
  ...
  Future<List<Map<String, dynamic>>> fetchAnnotations() async {
    try {
      final data = await supabase.rpc(
        'get_text_annotations_by_project_id',
      ...
      if (data is List) {
        ...
        if (listData.isNotEmpty && listData[0]['project_name'] != null) {
          setState(() {
            projectName = listData[0]['project_name'];
          });
        }
        return listData;
      } else {
